...
execute store result score #btz dummy run data get entity @e[type=marker,limit=1,tag=tornbreak] Pos[2] 1000
...
execute store result entity @e[type=marker,limit=1,tag=tornbreak] Pos[0] double 0.001 run scoreboard players get #btx dummy
...
execute at @e[type=marker,limit=1,tag=tornbreak] if score #breakpoint dummy matches 1.. if block ~ ~ ~ air run function weather:tornussy/refresh_breakpoint