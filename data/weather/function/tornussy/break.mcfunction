...
execute run scoreboard players set #breakpoint dummy 15
...
execute if score #rng dummy matches 0..90 at @s run function weather:tornussy/create_breakpoint
execute run kill @e[type=marker,tag=tb_refresh]

execute if entity @s[tag=BREAK_VALID] if score #rng dummy matches 75..90 at @e[type=marker,tag=tornbreak] run function weather:tornussy/break_weak
execute if entity @s[tag=BREAK_VALID] if score #rng dummy matches 45..74 at @e[type=marker,tag=tornbreak] run function weather:tornussy/break_medium
execute if entity @s[tag=BREAK_VALID] if score #rng dummy matches 0..44 at @e[type=marker,tag=tornbreak] run function weather:tornussy/break_strong

execute run scoreboard players reset #breakpoint dummy
execute run scoreboard players reset #bx dummy
execute run scoreboard players reset #by dummy
execute run scoreboard players reset #bz dummy
execute run scoreboard players reset #rng dummy
