...
execute if entity @s[tag=JOINTLEFT] at @s run summon item_display ~ ~ ~ {Tags:["tidgr"],item:{id:"minecraft:dirt",count:0}}
execute if entity @s[tag=JOINTLEFT] at @s run item replace entity @e[tag=tidgr,sort=nearest,limit=1] container.0 from entity @s weapon.offhand
execute if entity @s[tag=JOINTLEFT] at @s store result score @s weedburn run data get entity @e[tag=tidgr,sort=nearest,limit=1] item.components.minecraft:damage
execute if entity @s[tag=JOINTLEFT] at @s run kill @e[tag=tidgr,sort=nearest,limit=1]
