...
execute unless entity @s[tag=INITIAL_SPAWN] facing entity @e[gamemode=!creative,gamemode=!spectator,type=#core:tornado_valid,predicate=core:can_see_sky,tag=!tornado,sort=nearest,distance=..60,limit=1] feet rotated ~ 0 positioned ^ ^ ^0.66 run teleport @e[sort=nearest,limit=1,tag=tornadoang] ~ ~ ~ ~ ~
execute if entity @s[tag=INITIAL_SPAWN] if entity @a[predicate=core:can_see_sky,sort=nearest,distance=..24,limit=1] run tag @s remove INITIAL_SPAWN
