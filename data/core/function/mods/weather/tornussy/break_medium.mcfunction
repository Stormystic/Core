execute if block ~ ~ ~ #core:tornado_medium run tag @s add DO_BREAK
execute if entity @s[tag=DO_BREAK] if block ~ ~ ~ minecraft:stone run setblock ~ ~ ~ minecraft:cobblestone
execute if entity @s[tag=DO_BREAK] if block ~ ~ ~ minecraft:dirt run setblock ~ ~ ~ minecraft:coarse_dirt
...
execute unless entity @s[tag=BROKEN] run function core:mods/weather/tornussy/break_weak