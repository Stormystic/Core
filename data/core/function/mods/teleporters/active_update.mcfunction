...
$execute if score RELOADING teleDStorage matches ..0 as @s[tag=TELE_SYNCED,tag=!UPDATING_TELE,tag=!REFRESHING] positioned $(x) $(y) $(z) align xyz positioned ~.5 ~.5 ~.5 run execute unless entity @e[type=minecraft:item_display,distance=..0.6,tag=TELE_SYNCED,limit=1] run function core:mods/teleporters/desync with storage minecraft:teleupdate
...
$execute if score RELOADING teleDStorage matches ..0 as @s[tag=TELE_SYNCED,tag=!UPDATING_TELE,tag=!REFRESHING] positioned $(x) $(y) $(z) align xyz positioned ~.5 ~1.3 ~.5 run execute as @e[tag=!TELEPORTED,type=!#teleporters:allow_warping,distance=..0.6] run function core:mods/teleporters/teleport_entity with storage minecraft:teleupdate